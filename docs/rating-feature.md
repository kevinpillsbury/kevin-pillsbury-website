...
| **.gitignore** | `scripts/books.csv`, `scripts/books-cleaned.csv` so large data files aren’t committed. |
...
| ✅ | `scripts/README-rating-head.md` — how to run training (venv, install, `--csv`, `--output`, `--limit`, etc.). |
...
| ✅ | `.gitignore` — `scripts/books.csv`, `scripts/books-cleaned.csv`. |
...
1. **Run training (one-time)**  
   - From project root:  
     ...
     `python scripts/train-rating-head.py --csv scripts/books-cleaned.csv --output src/lib/rating-head.json`  
...
- **Command:**  
  `python scripts/train-rating-head.py --csv scripts/books-cleaned.csv --output src/lib/rating-head.json [--limit N]`  
